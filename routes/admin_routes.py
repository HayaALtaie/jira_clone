...
@admin_required
def admin_dashboard():
    if 'user' in session and session.get('role') == 'admin':  # تحقق من الجلسة
        users = User.query.all()  # استرجاع جميع المستخدمين
        return render_template('admin_dashboard.html', users=users)
    else:
        return redirect(url_for('auth.login'))  # إعادة التوجيه إلى صفحة تسجيل الدخول إذا لم يكن المستخدم إدمن
